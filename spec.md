# Plantegia — Product Specification v2

## Vision

**Plantegia** — приложение для планирования ротации растений в ограниченном пространстве.
...
**Plantegia решает эту проблему** — показывает временную шкалу всех растений и помогает спланировать ротацию так, чтобы пространство использовалось максимально эффективно. И бонусом прогнозирует сколько шишек будет на выходе.
...
### Эстетические ограниения:
Палитра: 4 цвета — тёмно-синий фон (#1A1A2E), приглушённый зелёный, оранжевый, бирюзовый для акцентов. Типографика: один моноширинный шрифт (Space Mono или IBM Plex Mono), два размера (14px/18px). Компоненты: только Hotbar, Inspector-popup, Canvas — никаких модалок, таблиц, сайдбаров. Формы: inline-редактирование прямо в Inspector, максимум 2-3 поля за раз. Layout: mobile-only 390px, вертикальная ориентация даже на десктопе. Анимации: только opacity/scale transitions, никакой физики. Иконки: текстовые символы (▢, ✕, ⚙) вместо SVG.
---